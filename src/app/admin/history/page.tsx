  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 max-w-5xl mx-auto">
      <header className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-black text-white">Generation History</h1>
        <p className="text-muted-foreground mt-1 text-sm">Recently processed files and status updates.</p>
      </header>

      <div className="glass-card rounded-2xl overflow-hidden bg-white/5 border border-white/5">
        <table className="w-full text-sm text-left">
          <thead>
            <tr className="bg-muted/30 border-b border-border">
              <th className="px-6 py-4 font-semibold text-white">Date</th>
              <th className="px-6 py-4 font-semibold text-white">Filename</th>
          ...
          <tbody>
            {files.map((f) => (
              <tr key={f.id} className="border-b border-border/50 hover:bg-white/5 transition-colors">
                ...
                <td className="px-6 py-4 font-medium text-white">{f.filename || f.drive_id}</td>
